...
from odes.core import evaluator_utils
...
class Evaluator:
    ...
    def __init__(self,
        ...
        self.full_model = isinstance(self.model, OdesModel)
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        if self.full_model:
            ...
            if box_rep not in ['box_3d', 'box_8c', 'box_8co',   ## benz
            ...
            if box_rep in ['box_8c', 'box_8co', 'box_4c', 'box_4ca']:  # benz
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            if validation:
                ...
                # Save proposals benz, currently proposals are still 3D bounding
                # boxes
                ...
                predictions_and_scores = \
                    self.get_avod_predicted_boxes_2d_and_scores(predictions,
                                                                box_rep)
                np.savetxt(avod_file_path, predictions_and_scores, fmt='%.5f')

                if self.full_model:
                    if box_rep in ['box_3d', 'box_4ca','box_2d']:
    ...
    def _update_avod_box_cls_loc_orient_losses(self,
        ...
        avod_classification_loss = \
            eval_losses[OdesModel.LOSS_FINAL_CLASSIFICATION]
        avod_regression_loss = \
            eval_losses[OdesModel.LOSS_FINAL_REGRESSION]

        avod_localization_loss = \
            eval_losses[OdesModel.LOSS_FINAL_LOCALIZATION]
        # avod_orientation_loss = \
        #    eval_losses[OdesModel.LOSS_FINAL_ORIENTATION]
        ...
        # (greater than 0)
        if (avod_localization_loss > 0.0) : # and (avod_orientation_loss > 0.0):
    ...
    def save_prediction_losses_results(self,
        ...
        elif box_rep in ['box_3d', 'box_4ca']:
            summary_utils.add_scalar_summary(
    ...
    def get_cls_accuracy(self,
        ...
        if self.full_model:
            classification_pred = \
                predictions[OdesModel.PRED_MB_CLASSIFICATION_SOFTMAX]
            classification_gt = \
                predictions[OdesModel.PRED_MB_CLASSIFICATIONS_GT]
    ...
    def get_avod_predicted_boxes_2d_and_scores(self, predictions,
                                               box_rep):
        ...
        """
        if box_rep == 'box_2d':    ## benz
            final_pred_anchors = predictions[
                OdesModel.PRED_TOP_PREDICTION_ANCHORS]
            final_pred_boxes_2d = final_pred_anchors
        ...
        final_pred_softmax = predictions[
            OdesModel.PRED_TOP_CLASSIFICATION_SOFTMAX]
        ...
        for pred_idx in range(len(final_pred_boxes_2d)):
    ...
    def get_avod_predicted_boxes_3d_and_scores(self, predictions,
        ...
        if box_rep == 'box_2d':    ## benz
            ...
            final_pred_orientations = predictions[
                OdesModel.PRED_TOP_ORIENTATIONS]
            ...
            final_pred_boxes_3d = predictions[
                OdesModel.PRED_TOP_PREDICTION_BOXES_3D]
            ...
            inal_pred_boxes_3d[above_pi_indices, 6] -= two_pi
    ...
    def get_avod_predicted_box_corners_and_scores(self,
        ...
        if box_rep in ['box_8c', 'box_8co']:
            final_pred_box_corners = predictions[OdesModel.PRED_TOP_BOXES_8C]
        elif box_rep in ['box_4c', 'box_4ca']:
            final_pred_box_corners = predictions[OdesModel.PRED_TOP_BOXES_4C]
