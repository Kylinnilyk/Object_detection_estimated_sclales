...
from odes.core.mini_batch_preprocessor import MiniBatchPreprocessor
from odes.core.minibatch_samplers import balanced_positive_negative_sampler
...
class MiniBatchUtils:
    ...
    def get_file_path(self, classes_name, anchor_strides, sample_name):
        ...
        if sample_name:
            return self.mini_batch_dir + '/' + classes_name + \
                '[ ' + anchor_strides_str + ']/' + \
                sample_name + ".npy",  self.mini_batch_dir + '/' + classes_name + 'img/' + sample_name + ".npy",\
                self.mini_batch_dir + '/' + classes_name + 'img_norm/' + sample_name + ".npy"



        return self.mini_batch_dir + '/' + classes_name + \
            '[ ' + anchor_strides_str + ']' , self.mini_batch_dir + '/' + classes_name + 'img', \
            self.mini_batch_dir + '/' + classes_name + 'img_norm'
    ...
    def get_anchors_info(self, classes_name, anchor_strides, sample_name):
        ...
        """
        file_name, _, _ = self.get_file_path(classes_name, anchor_strides,
                                       sample_name)
    ...
    def get_img_roi_data(self, classes_name, anchor_strides, sample_name):
        """Reads in the file containing the information matrix
        ...
        Returns:
            ...
            [] if the file contains an empty array
        """
        _,  img_roi_name, img_roi_norm_name = self.get_file_path(classes_name, anchor_strides,
                                       sample_name)

        if not os.path.exists(img_roi_name):
            raise FileNotFoundError(
                "{} not found for sample {} in {}, "
                "run the preprocessing script first".format(
                    img_roi_name,
        ...
        img_roi = np.load(img_roi_name)

        if not os.path.exists(img_roi_norm_name):
            raise FileNotFoundError(
                "{} not found for sample {} in {}, "
                "run the preprocessing script first".format(
                    img_roi_norm_name,
        ...
        img_roi_norm = np.load(img_roi_norm_name)


        if img_roi.any() and img_roi_norm.any() :
            return np.asarray(img_roi,dtype=np.float32), np.asarray(img_roi_norm, dtype=np.float32)
        return []
