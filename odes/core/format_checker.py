...
def check_input_img_roi(input_data):
    """check the format and boundary of input data are correct or not.

    Args:
        input_data: input array, every entry [x1, y1, x2, y2] should satisfy that x1 < x2, and y1 < y2
    """
    ## checking input data
    if isinstance(input_data, np.ndarray):
        if np.where( input_data < 0 )[0].size :
            raise ValueError('img_roi shoud be positive')

        if np.where( input_data[:,0] > input_data[:,2] )[0].size :
            raise ValueError('x1 should be less than x2 in img_roi')

        if np.where( input_data[:,1] > input_data[:,3] )[0].size :
            raise ValueError('y1 should be less than y2 in img_roi')
    else:
        raise TypeError('Given input is not array')


def check_input_img_roi_norm(input_data):
    """ check the format and boundary of input data are correct or not.
    Args:
        input_data: input array, and  all the data should be in the range of [0,1],
                every entry [x1, y1, x2, y2] should satisfy that x1 < x2 and y1 < y2
    """

    # checking input data
    if isinstance(input_data, np.ndarray):
        if np.where( input_data < 0 )[0].size :
            raise ValueError('img_roi_norm shoud be 0 and 1')

        if np.where( input_data > 1 )[0].size :
            raise ValueError('img_roi_norm shoud be 0 and 1')

        if np.where( input_data[:,0] > input_data[:,2] )[0].size :
            raise ValueError('x1 should be less than x2 in img_roi_norm')

        if np.where( input_data[:,1] > input_data[:,3] )[0].size :
            raise ValueError('y1 should be less than y2 in img_roi_norm')
        #cretia1 = np.where(img_roi_norm < 0)
        #assert not cretia1[0].size,'input data is not correct'
        #cretia2 = np.where(img_roi_norm > 1)
        #assert not cretia2[0].size, 'input data is not correct'
        #cretia3 = np.where(img_roi_norm[:,0]>img_roi_norm[:,2])
        #assert not cretia3[0].size, 'input data is not correct'
        #cretia4 = np.where(img_roi_norm[:,1]>img_roi_norm[:,3])
        #assert not cretia4[0].size, 'input data is not correct'
    else:
        raise TypeError('Given input is not array')


def check_label_2d_boxes(input_data):
    """checks the format and data in 2d_boxes_gt are correct or not.

    Args:
        input_data: all the value should be positive, and
        every entry [x1, y1, x2, y2] should satify that x1 < x2 and y1 < y2
    """
    ## checking the input_data
    if isinstance(input_data, np.ndarray):
        if np.where( input_data < 0 )[0].size :
            raise ValueError('2d_boxes_gt shoud be positive')

        if np.where( input_data[:,0] > input_data[:,2] )[0].size :
            raise ValueError('x1 should be less than x2 in 2d_boxes_gt')

        if np.where( input_data[:,1] > input_data[:,3] )[0].size :
            raise ValueError('y1 should be less than y2 in 2d_boxes_gt')
    else:
        raise TypeError('Given input is not np.ndarray')
