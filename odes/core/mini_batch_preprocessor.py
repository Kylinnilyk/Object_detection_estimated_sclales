...
from odes.core import box_3d_encoder, anchor_projector
...
from odes.core import box_2d_encoder
...
class MiniBatchPreprocessor(object):
    ...
    def _calculate_img_anchors_info(self,
                                all_anchor_boxes_3d,
                                empty_anchor_filter,
                                gt_labels,
                                stereo_calib_p2,
        ...
        img_boxes_anchors, img_boxes_anchors_norm  = anchor_projector.project_3d_box_to_image_space(anchor_boxes_3d, stereo_calib_p2, image_size)  ## project 3d_boxes
        ...
        for gt_idx in range(len(gt_labels)):
            ...
            #                                            ortho_rotate=True)
            offsets = anchor_encoder.np_2d_box_to_offset(anchors_to_update,
                                                      gt_box_2d)
    ...
    def preprocess(self, indices):
        ...
        output_dir, img_roi_dir, img_roi_norm_dir = self.mini_batch_utils.get_file_path(classes_name,
                                                         anchor_strides,
                                                         sample_name=None)
        ...
        for sample_idx in indices:
            ...
            if False and self._check_for_existing(classes_name, anchor_strides,     ## modification by benz to debug code
                                        sample_name):
            ...
            anchors_info, img_boxes_anchors, img_boxes_anchors_norm  = self._calculate_img_anchors_info(
                all_anchor_boxes_3d, empty_anchor_filter, filtered_gt_list, stereo_calib_p2, image_shape )
    ...
    def _save_to_file(self, classes_name, anchor_strides, sample_name,
                      ...
                      img_boxes_anchors = np.array([]),
                      img_boxes_anchors_norm = np.array([]) ):
        """
        ...
        all_info_name, img_roi_name, img_roi_norm_name = self.mini_batch_utils.get_file_path(classes_name,
                                                        anchor_strides,
                                                        sample_name)

        # Save to npy file
        anchors_info = np.asarray(anchors_info, dtype=np.float32)
        np.save(all_info_name, anchors_info)
        np.save(img_roi_name,img_boxes_anchors )
        np.save(img_roi_norm_name, img_boxes_anchors_norm)
