...
from odes.core import anchor_generator
...
def tile_anchors_3d(area_extents,
    ...
    # anchor_rotations = np.asarray([0, np.pi / 2.0]) ## benz
    anchor_rotations = np.asarray([0, np.pi/4.0, np.pi / 2.0, np.pi*3.0/4.0])
    # ## benz for car
    # anchor_rotations = np.asarray([0, np.pi / 2.0]) ## benz for people
