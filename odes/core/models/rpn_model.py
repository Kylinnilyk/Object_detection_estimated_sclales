...
from odes.builders import feature_extractor_builder
from odes.core import anchor_encoder
from odes.core import anchor_filter
from odes.core import anchor_projector
from odes.core import box_3d_encoder
from odes.core import constants
from odes.core import losses
from odes.core import model
from odes.core import summary_utils
from odes.core.anchor_generators import grid_anchor_3d_generator
from odes.datasets.kitti import kitti_aug
...
class RpnModel(model.DetectionModel):
    ...
    PL_LABEL_BOXES_2D_GT = 'label_boxes_2d_pl'   # benz
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('pl_labels'):
            ...
            self._add_placeholder(tf.float32, [None, 4],      ## benz
                                  self.PL_LABEL_BOXES_2D_GT)
        ...
        with tf.variable_scope('pl_anchors'):
            ...
            with tf.variable_scope('img_anchor_projections'):
                self._img_roi_pl = self._add_placeholder(tf.float32, [None, 4],
                                      self.PL_IMG_ANCHORS)
                self._img_anchors_norm_tf_order_pl = self._add_placeholder(
    ...
    def _set_up_feature_extractors(self):
        ...
        #self.bev_feature_maps, self.bev_end_points =    ## commented by benz, to remove bev_features
        ...
        #with tf.variable_scope('bev_bottleneck'):   ## commented by benz, to remove the bev_features
        ...
        #    self.bev_bottleneck = self._bev_input_batches    ## for output of bev_bottleneck has the same size with self._bev_input_batches, under the FPN mode.

        #TODO, got to increase the number of channels in img_bottleneck later
    ...
    def build(self):
        ...
        ## benz, I adopt more complex input feature, maybe i will out of memory
        ...
        with tf.variable_scope('proposal_roi_pooling'):
            with tf.variable_scope('box_indices'):
                ...
                img_boxes_norm_batches = tf.expand_dims(
                    self._img_anchors_norm_tf_order_pl, axis=0)
            ...
            img_proposal_rois = tf.image.crop_and_resize(
                img_proposal_input,
                self._img_anchors_norm_tf_order_pl,
                tf_box_indices,
                self._proposal_roi_crop_size)

        ## croped feature maps are input for following network
        ...
        with tf.variable_scope('anchor_predictor', 'ap', [rpn_fusion_out]):
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                reg_fc8 = slim.conv2d( cls_fc7_drop, # reg_fc7_drop,
                                      4,   ## use 4, we try regression on rpn directly first
                                      [1, 1],
                                      activation_fn=None,
                                      scope='reg_fc8')
        ...
        with tf.variable_scope('proposals'):
            ...
            with tf.variable_scope('decoding'):
                image_shape = tf.cast(tf.shape(self.placeholders[self.PL_IMG_INPUT])[0:2],tf.float32)  ##TODO, check it is raw shape or resize shape
                regressed_boxes = anchor_encoder.tf_2d_offset_to_box(
                          self._img_roi_pl, offsets)
                regressed_boxes_norm = anchor_encoder.tf_boxes_normalization( regressed_boxes, image_shape)## image_shape should be one before resizing operation
                regressed_boxes_norm_order = anchor_projector.reorder_projected_boxes(regressed_boxes_norm)
            ...
            with tf.variable_scope('nms'):
                ...
                top_indices = tf.image.non_max_suppression(
                    regressed_boxes_norm_order, objectness_scores,
                    max_output_size=self._nms_size,
                    iou_threshold=self._nms_iou_thresh)

                top_anchors = tf.gather(regressed_boxes, top_indices)
        ...
        with tf.variable_scope('img_rpn_rois'):
            # ROIs on image input
            mb_img_anchors_norm = tf.boolean_mask(self._img_anchors_norm_tf_order_pl,
                                                  mini_batch_mask)
            ...
            img_input_rois = tf.image.crop_and_resize(
                ...
                mb_img_anchors_norm,
                mb_img_box_indices,
                (32, 32))   ## benz 32 can be one important parameter
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        #TODO, read the ground truth 2d boxes into this tensor ----benz
        label_boxes_2d_gt = sample.get(constants.KEY_LABEL_BOXES_2D)
        fc.check_label_2d_boxes(label_boxes_2d_gt)

        img_roi = []
        img_roi_norm = []
        img_roi_all = sample.get(constants.KEY_IMG_ROI_ALL)
        if img_roi_all:
            img_roi, img_roi_norm = img_roi_all
            fc.check_input_img_roi_norm(img_roi_norm)
            fc.check_input_img_roi(img_roi)
        else:
            print('empty image_rois')
        #img_roi = sample.get(constants.KEY_IMG_ROI)
        #img_roi_norm = sample.get(constants.KEY_IMG_ROI_NORM)
        # Network input data
        ...
        self._placeholder_inputs[self.PL_IMG_ANCHORS] = img_roi

        # self._img_anchors_norm = img_roi_norm[:, [1, 0, 3, 2]]
        self._placeholder_inputs[self.PL_IMG_ANCHORS_NORM] = img_roi_norm[:, [1, 0, 3, 2]]
        ...
        # Fill in the rest,, benz, I kick some input off
        # self._placeholder_inputs[self.PL_BEV_INPUT] = bev_input # benz
        self._placeholder_inputs[self.PL_IMG_INPUT] = image_input

        # self._placeholder_inputs[self.PL_LABEL_ANCHORS] = label_anchors
        # self._placeholder_inputs[self.PL_LABEL_BOXES_3D] = label_boxes_3d #benz
        self._placeholder_inputs[self.PL_LABEL_CLASSES] = label_classes
        self._placeholder_inputs[self.PL_LABEL_BOXES_2D_GT] = label_boxes_2d_gt
    ...
    def _fill_anchor_pl_inputs(self,
        ...
        # reading the img_anchors directly, instead of computing
    ...
    def loss(self, prediction_dict):
        ...
        with tf.variable_scope('rpn_losses'):
            ...
            with tf.variable_scope('total_loss'):  # benz, in the late, I will cut this localization_loss
