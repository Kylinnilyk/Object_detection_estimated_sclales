...
def flip_roi(rois, image_shape):
    ...
    args:
        ...
        image_shape: [h, w]
    """
    x1 = image_shape[1] - rois[:,2]
    x2 = image_shape[1] - rois[:,0]
    rois[:,0] = x1
    rois[:,2] = x2
    return rois


def flip_roi_norm(roi_norm):
    """
    flip the roi_norm horizontally
    args:
        rois_norm: [x1, y1, x2, y2]
    """
    x1 = 1 - roi_norm[:,2]
    x2 = 1 - roi_norm[:,0]
    roi_norm[:,0] = x1
